...
def read_afinn():
    ...
    afinn = dict()
    with open('afinndict.txt', 'r') as dataval:
        for line in dataval:
...
def get_tweets(name):
    ...
    with open(name + '.pkl', 'rb') as dataval:
        return pickle.load(dataval)
...
def main():
    ...
    classif = classify_results(pos_tweets, neg_tweets, neutral_tweets)
    save_obj(classif, 'classify')
